...
class ActiveAerobicScreen extends StatefulWidget {
  final AerobicSessionStructure session;

  const ActiveAerobicScreen({Key? key, required this.session}) : super(key: key);

  @override
  State<ActiveAerobicScreen> createState() => _ActiveAerobicScreenState();
}

class _ActiveAerobicScreenState extends State<ActiveAerobicScreen> {
  // Estados do Timer
  ...
  // Controle Lógico
  int _currentRep = 0;
  bool _isRunningPhase = false; // true = correr/força, false = caminhar/recuperação
  bool _isWarmup = true;
  ...
  void _nextPhase() {
    ...
    if (_isWarmup) {
      // Fim do Aquecimento -> Começa 1ª Repetição (Correr/Força)
      _isWarmup = false;
      _currentRep = 1;
      _startRunPhase();
    } else if (_isCooldown) {
      ...
      if (_isRunningPhase) {
        // Acabou de correr/força -> Caminhar/Recuperação (ou Cooldown se acabou reps)
        if (widget.session.recoveryIntervalSeconds > 0) {
           _startWalkPhase();
        } else {
           // Se não tem recuperação (corrida continua), verifica reps
           _checkRepsOrFinish();
        }
      } else {
        // Acabou de caminhar/recuperação -> Verificar se tem mais repetições
        _checkRepsOrFinish();
      }
    }
  }
  ...
  void _startRunPhase() {
    ...
    // Texto dinâmico
    _phaseName = widget.session.type == AerobicType.running ? "CORRER" : "FORÇA / RPM ALTO";
    _phaseColor = AppColors.neonGreen;
    _phaseSecondsRemaining = widget.session.mainIntervalSeconds;
    // Vibrar ou som aqui seria ideal
  }

  void _startWalkPhase() {
    _isRunningPhase = false;
    _phaseName = widget.session.type == AerobicType.running ? "CAMINHAR" : "RECUPERAÇÃO";
    _phaseColor = Colors.blue;
    _phaseSecondsRemaining = widget.session.recoveryIntervalSeconds;
  }

  void _startCooldown() {
    _isCooldown = true;
    _phaseName = "WASHOUT / DESAQ.";
        ...
        builder: (context, setDialogState) {
          ...
          return AlertDialog(
            ...
            actions: [
              ...
              ElevatedButton(
                onPressed: () async {
                  await context.read<AerobicProvider>().completeSession(
                    widget.session.type, // Passa o tipo
                    score, 
                    _totalSeconds
                  );
                  if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    if (_isWarmup) {
      ...
      phaseTotalSeconds = widget.session.mainIntervalSeconds;
    } else {
      phaseTotalSeconds = widget.session.recoveryIntervalSeconds;
    }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(widget.session.type == AerobicType.running ? 'Modo Corrida' : 'Modo Bike'),
