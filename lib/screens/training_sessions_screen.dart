        ...
        return SafeArea(
          child: Column(
            ...
            children: [
              ...
              ListTile(
                leading: Icon(Icons.color_lens, color: AppColors.neonPurple),
...
// --- ABA CONFIGURAÇÃO MUSCULAÇÃO ---
class _BodyBuildingConfigTab extends StatelessWidget {
  const _BodyBuildingConfigTab();
  ...
  Future<void> _editSessionName(
    ...
    final result = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              if (nameController.text.trim().isNotEmpty) Navigator.of(context).pop(true);
            },
    ...
    if (result == true && nameController.text.trim().isNotEmpty) {
      await provider.updateTrainingSession(session.id, nameController.text.trim(), session.description);
    }
  ...
  Widget build(BuildContext context) {
    return Consumer3<TrainingSessionProvider, MuscleGroupProvider, ThemeProvider>(
      builder: (context, trainingProvider, muscleProvider, themeProvider, _) {
        ...
        return Scaffold( // Scaffold interno para o FAB
              ...
              ? Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text('Nenhuma sessão de treino', style: Theme.of(context).textTheme.titleLarge),
                      const SizedBox(height: 8),
                      Text('Crie sua primeira sessão de treino', style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: Colors.grey[600])),
                    ],
                  ),
                )
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return NeonCard(
                      ...
                      child: ListTile(
                        ...
                        title: Text(session.name, style: Theme.of(context).textTheme.titleLarge),
                        subtitle: Text('Criado em ${session.createdAt.day}/${session.createdAt.month}/${session.createdAt.year}', style: Theme.of(context).textTheme.bodySmall),
                        trailing: Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            PopupMenuButton<String>(
                              ...
                              itemBuilder: (context) => [
                                const PopupMenuItem(value: 'edit', child: Row(children: [Icon(Icons.edit, size: 20), SizedBox(width: 8), Text('Editar nome')])),
                        ...
                        onTap: () {
                          Navigator.push(context, MaterialPageRoute(builder: (context) => TrainingDetailScreen(sessionId: session.id)));
                        },
                      ),
                    );
                  },
          ...
          floatingActionButton: FloatingActionButton(
            onPressed: () {
              Navigator.push(context, MaterialPageRoute(builder: (context) => const CreateTrainingSessionScreen()));
            },
